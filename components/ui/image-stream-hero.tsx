...
export type StreamImage = {
  ...
  alt?: string;
  /** Optional title for interactive event cards */
  title?: string;
  /** Optional tag/badge (e.g. "@dev", "@leo9") */
  tag?: string;
  ...
  href?: string;
  /** Explicit video indicator or fallback */
  isVideo?: boolean;
  /** Optional poster image for video covers */
  poster?: string;
};
...
export type ImageStreamHeroProps = {
  ...
  path?: CorridorPath;
  /** Pause animation when any card or the corridor is hovered. @default true */
  pauseOnHover?: boolean;
  /** Callback when an item is clicked */
  onCardClick?: (item: StreamImage, index: number) => void;
...
export function ImageStreamHero({
  ...
  const css = React.useMemo(
    () =>
      ...
      // Pausing on hover stops motion when any card is hovered
      (pauseOnHover
        ? `.${container}:has(.${card}:hover) .${card}, .${card}:hover { animation-play-state: paused !important; }`
        : "") +
    ...
    [right, left, card, container, p, pauseOnHover],
  );
        ...
        >
          {[right, left].map((name) =>
            Array.from({ length: cards }, (_, i) => {
              ...
              const isVideo = img?.isVideo || (img?.src ? isVideoUrl(img.src) : false);

              return (
                <div
                  key={`${name}-${i}`}
                  onClick={() => img && onCardClick?.(img, i)}
                  className={cn(
                    card,
                    "absolute overflow-hidden pointer-events-auto cursor-pointer transition-shadow duration-200 hover:shadow-2xl hover:ring-1 hover:ring-white/30",
                  )}
                  style={{
                    left: "50%",
                    top: `${axis}%`,
                ...
                >
                  ...
                  {/* Optional floating tag pill (e.g. @dev, @leo9) matching the reference screenshot */}
                  {img?.tag && (
                    <div className="absolute top-[3%] left-[4%] z-20 pointer-events-none">
                      <span
                        className={cn(
                          "inline-block px-[0.6cqw] py-[0.2cqw] rounded-full text-[0.8cqw] font-bold tracking-tight shadow-md backdrop-blur-sm",
                          img.tagColor || "bg-black/75 text-white",
                        )}
                      >
                        {img.tag}
                      </span>
