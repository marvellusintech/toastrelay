"use client";

import React from "react";
import Link from "next/link";
import { ArrowUpRight } from "lucide-react";
import { cn } from "@/lib/utils";

function ToastrelayLogoIcon({ className = "w-4 h-4" }: { className?: string }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      viewBox="0 0 859 867"
      fill="currentColor"
    >
      <path
        fillRule="evenodd"
        d="M854.363,351.300 C680.101,355.561 548.211,457.780 427.319,322.050 C402.012,293.638 377.681,239.638 390.269,175.802 C398.068,117.308 405.870,58.797 413.669,0.304 C484.138,-1.459 592.187,13.495 624.266,47.103 C618.961,78.902 584.713,107.709 567.717,130.952 C528.071,186.196 488.414,241.456 448.768,296.700 C447.747,316.248 448.193,321.888 456.568,333.750 C459.818,335.050 463.069,336.350 466.318,337.650 C500.673,346.151 529.644,302.240 550.167,286.950 C614.510,238.206 678.872,189.447 743.215,140.702 C789.641,167.166 850.305,282.178 854.363,351.300 ZM0.274,386.399 C-1.107,323.152 24.620,264.236 49.023,222.601 C51.623,222.601 54.223,222.601 56.823,222.601 C77.616,241.093 222.695,350.461 245.971,341.550 C251.170,337.000 256.371,332.449 261.571,327.900 C261.681,250.434 147.663,192.355 127.022,127.052 C159.315,80.573 226.799,38.978 288.870,23.704 C297.948,177.379 362.861,288.632 230.371,374.699 C203.076,392.430 154.519,412.337 101.673,401.999 C67.876,396.799 34.070,391.599 0.274,386.399 ZM858.263,495.598 C855.444,583.542 803.896,660.524 760.765,712.045 C758.165,712.045 755.564,712.045 752.965,712.045 C696.122,665.061 632.989,622.941 569.667,583.347 C540.519,565.121 517.520,524.801 474.118,522.897 C466.969,529.397 459.818,535.898 452.668,542.397 C452.836,577.821 576.891,727.539 602.817,758.845 C615.815,773.793 628.818,788.746 641.816,803.694 C637.931,786.479 612.551,867.709 429.269,866.093 C418.870,802.400 408.468,738.688 398.069,674.996 C387.879,622.798 407.871,576.893 425.369,550.197 C520.457,405.124 683.008,494.275 858.263,495.598 ZM0.274,491.698 C51.396,484.284 83.940,479.539 121.172,472.198 C160.738,464.397 194.820,480.403 218.671,489.748 C356.626,543.796 322.770,692.952 304.470,846.593 C302.520,846.593 300.570,846.593 298.620,846.593 C255.370,832.310 146.798,786.985 138.722,745.195 C179.668,689.301 220.625,633.390 261.571,577.497 C273.836,558.450 267.778,534.293 247.921,526.797 C212.556,513.448 103.357,647.451 66.573,655.496 C43.101,660.629 1.041,527.652 0.274,491.698 Z"
      />
    </svg>
  );
}

export function isColorDark(hex?: string): boolean {
  if (!hex) return false;
  const cleanHex = hex.replace("#", "").trim();
  if (cleanHex.length !== 6 && cleanHex.length !== 3) return false;
  const r = parseInt(
    cleanHex.length === 3 ? cleanHex[0] + cleanHex[0] : cleanHex.slice(0, 2),
    16,
  );
  const g = parseInt(
    cleanHex.length === 3 ? cleanHex[1] + cleanHex[1] : cleanHex.slice(2, 4),
    16,
  );
  const b = parseInt(
    cleanHex.length === 3 ? cleanHex[2] + cleanHex[2] : cleanHex.slice(4, 6),
    16,
  );
  const brightness = (r * 299 + g * 587 + b * 114) / 1000;
  return brightness < 128;
}

export interface EventMinimalFooterProps {
  isDark?: boolean;
  className?: string;
}

export function EventMinimalFooter({
  isDark = false,
  className,
}: EventMinimalFooterProps) {
  const currentYear = new Date().getFullYear();

  return (
    <footer
      role="contentinfo"
      className={cn(
        "w-full pt-10 pb-12 transition-colors font-sans",
        isDark
          ? "bg-stone-950 border-t border-stone-800/80 text-stone-400"
          : "bg-neutral-950 border-t border-neutral-800 text-neutral-400",
        className,
      )}
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="flex flex-col gap-8">
          {/* Main Top Row */}
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            {/* Brand section */}
            <div className="flex flex-wrap items-center gap-3 sm:gap-4">
              <Link
                href="/"
                className="inline-flex items-center gap-2.5 group focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-neutral-400 rounded-lg"
              >
                <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-white text-black transition-transform group-hover:scale-105 shadow-sm">
                  <ToastrelayLogoIcon className="w-4 h-4" />
                </div>
                <span
                  className={cn(
                    "text-lg uppercase font-display font-bold tracking-tight transition-colors",
                    isDark
                      ? "text-stone-100 group-hover:text-white"
                      : "text-white group-hover:text-neutral-200",
                  )}
                >
                  Toastrelay
                </span>
              </Link>

              <span
                className={cn(
                  "hidden sm:inline-block h-3.5 w-px",
                  isDark ? "bg-stone-800" : "bg-neutral-800",
                )}
                aria-hidden="true"
              />

              <span
                className={cn(
                  "text-xs font-normal",
                  isDark ? "text-stone-400" : "text-neutral-400",
                )}
              >
                Every event deserves a stage
              </span>
            </div>

            {/* Host CTA Button */}
            <div>
              <Link
                href="/create-account"
                className={cn(
                  "inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-semibold tracking-wide transition-all shadow-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2",
                  isDark
                    ? "bg-stone-900/90 text-stone-200 border border-stone-800 hover:bg-stone-800 hover:text-white hover:border-stone-700 focus-visible:ring-stone-400"
                    : "bg-neutral-900 text-neutral-200 border border-neutral-800 hover:bg-neutral-800 hover:text-white hover:border-neutral-700 focus-visible:ring-neutral-400",
                )}
              >
                <span>Host your own event</span>
                <ArrowUpRight className="w-3.5 h-3.5 opacity-70" />
              </Link>
            </div>
          </div>

          {/* Bottom Bar: Copyright & Links */}
          <div
            className={cn(
              "pt-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 text-xs border-t",
              isDark
                ? "border-stone-900 text-stone-500"
                : "border-neutral-900 text-neutral-500",
            )}
          >
            <p>&copy; {currentYear} Toastrelay. All rights reserved.</p>

            <nav aria-label="Event footer navigation">
              <ul className="flex flex-wrap items-center gap-5 sm:gap-6">
                <li>
                  <Link
                    href="/discovery"
                    className={cn(
                      "transition-colors",
                      isDark
                        ? "text-stone-400 hover:text-stone-200"
                        : "text-neutral-400 hover:text-white",
                    )}
                  >
                    Discover Events
                  </Link>
                </li>
                <li>
                  <Link
                    href="/terms"
                    className={cn(
                      "transition-colors",
                      isDark
                        ? "text-stone-400 hover:text-stone-200"
                        : "text-neutral-400 hover:text-white",
                    )}
                  >
                    Terms
                  </Link>
                </li>
                <li>
                  <Link
                    href="/privacy"
                    className={cn(
                      "transition-colors",
                      isDark
                        ? "text-stone-400 hover:text-stone-200"
                        : "text-neutral-400 hover:text-white",
                    )}
                  >
                    Privacy
                  </Link>
                </li>
              </ul>
            </nav>
          </div>
        </div>
      </div>
    </footer>
  );
}

export default EventMinimalFooter;
